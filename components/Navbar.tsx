  ...
  const LanguageSwitcher = ({ isMobile = false }) => (
    <div className={`flex items-center ${isMobile ? 'gap-4 text-xl' : 'gap-1.5 md:gap-2 text-[9px] md:text-[10px]'}`}>
      <button 
        ...
        className={`font-black tracking-widest transition-colors ${language === 'en' ? 'text-[#6D28D9]' : 'text-[#1E1B4B]/40 hover:text-[#1E1B4B]'}`}
      >
        EN
      </button>
      ...
      <button 
        ...
        className={`font-black tracking-widest transition-colors ${language === 'zh' ? 'text-[#6D28D9]' : 'text-[#1E1B4B]/40 hover:text-[#1E1B4B]'}`}
      >
        中文
      </button>
    </div>
    ...
    <div className="fixed top-0 left-0 right-0 z-50 pointer-events-none">
      ...
      <div className={`bg-[#1E40AF] text-white py-3 px-6 md:px-8 pointer-events-auto hidden lg:block transition-all duration-500 ${isScrolled ? 'opacity-0 -translate-y-full' : 'opacity-100'}`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex gap-8 items-center">
            <a href="tel:9899899899" className="flex items-center gap-2 text-[9px] uppercase tracking-[0.2em] font-black hover:text-[#D1D5DB] transition-colors">
              <Phone className="w-3 h-3 text-white/70" /> 989-989-9899
            </a>
            <a href="mailto:book@boomboomthailand.com" className="flex items-center gap-2 text-[9px] uppercase tracking-[0.2em] font-black hover:text-[#D1D5DB] transition-colors border-l border-white/20 pl-8">
      ...
      <nav className={`px-6 md:px-8 transition-all duration-500 ${isScrolled ? 'py-3 md:py-4' : 'py-6 md:py-10'}`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          {/* Branding - Left Side */}
          <div className={`pointer-events-auto relative group cursor-pointer transition-all duration-500 ${isScrolled ? 'scale-95 origin-left' : 'scale-100 origin-left'}`}>
          ...
          <div className={`hidden md:flex pointer-events-auto items-center gap-8 lg:gap-10 bg-white/80 backdrop-blur-xl shadow-[0_10px_40px_rgba(109,40,217,0.1)] border border-[#1E1B4B]/10 px-8 py-4 rounded-full transform transition-all hover:scale-[1.02] ${isScrolled ? 'shadow-md' : 'shadow-xl'}`}>
            {navLinks.map(item => (
              <a key={item.name} href={item.href} className="text-[10px] lg:text-xs uppercase tracking-widest font-black hover:text-[#6D28D9] transition-colors text-[#1E1B4B]">
                {item.name}
              </a>
            ))}
            <div className="h-4 w-px bg-[#1E1B4B]/10" />
            <button className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#1E1B4B] hover:text-[#1E40AF] transition-colors">
      ...
      <div className={`fixed inset-0 bg-[#1E1B4B] z-[100] transition-all duration-700 ease-in-out pointer-events-auto flex flex-col ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full'}`}>
        ...
        <div className="p-12">
          <button className="w-full bg-[#6D28D9] py-5 rounded-full text-white uppercase tracking-widest font-black text-xs">
